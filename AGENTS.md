...
- The **`feat/item-get-edit-list` branch and its `@takescake` naming** are
  upstream-PR territory — do not "fix" them to `@u2giants` (see [Intentional quirks](#intentional-quirks-and-non-obvious-decisions)).
...
Actually:
It is deliberate. `feat/item-get-edit-list` backs the **upstream pull request**
([CakeRepository#7](https://github.com/CakeRepository/1Password-MCP/pull/7)) and
must keep that project's `@takescake` name. `main` is the **fork's published
branch** (the default and only working branch) and uses the `@u2giants` scope the
owner controls on npm.

Why:
The owner cannot publish under `@takescake` (not their scope) but still wants to
contribute the same changes upstream. Splitting by branch satisfies both.

Do not change because:
Renaming `feat/item-get-edit-list` to `@u2giants` would corrupt the upstream PR;
renaming `main` to `@takescake` would make `npm publish` fail (wrong scope).
`release.yml` enforces this with a guard that refuses to publish anything not
named `@u2giants/1password-mcp`.
