...
- The **`feat/item-get-edit-list` branch and its `@takescake` naming** are the
  historical record of upstream PR #7 — do not "fix" them to `@u2giants` (see
  [Intentional quirks](#intentional-quirks-and-non-obvious-decisions)).
...
Looks like:
The package is named `@u2giants/1password-mcp` on `main` but
`@takescake/1password-mcp` on `feat/item-get-edit-list` — looks like a botched
rename.

Actually:
It is deliberate. `feat/item-get-edit-list` backed the **upstream pull request**
([CakeRepository#7](https://github.com/CakeRepository/1Password-MCP/pull/7)) and
keeps that project's `@takescake` name. `main` is the **fork's published branch**
(the default and only working branch) and uses the `@u2giants` scope the owner
controls on npm.

Why:
The owner cannot publish under `@takescake` (not their scope) but still wants to
contribute the same changes upstream. Splitting by branch satisfied both.

Do not change because:
Renaming `feat/item-get-edit-list` to `@u2giants` would rewrite the record of a
real upstream PR; renaming `main` to `@takescake` would make `npm publish` fail
(wrong scope). `release.yml` enforces this with a guard that refuses to publish
anything not named `@u2giants/1password-mcp`.

Current status (2026-07-15): **PR #7 is closed, not merged** — the maintainer
re-applied its item tools upstream through his own
[PR #11](https://github.com/CakeRepository/1Password-MCP/pull/11) (merged
2026-07-04). `feat/item-get-edit-list` is therefore now historical; it is not
the base for any open PR. See [Upstream contributions](#upstream-contributions)
for how new contributions are made.

### Upstream contributions

New contributions to `CakeRepository/1Password-MCP` are made from **fresh
branches cut off `upstream/master`** (not off `main`, which carries fork-only
plumbing — `@u2giants` rename, OIDC publishing, `vibe_coding` defaults — that
must never go upstream). Genericize any owner-specific default before proposing
it (e.g. the `op_run` vault allow-list defaults to `vibe_coding` on this fork but
to *unrestricted* in the upstream PR).

Open upstream PRs as of 2026-07-15:

| PR | Branch | Contents | Notes |
|---|---|---|---|
| [#12](https://github.com/CakeRepository/1Password-MCP/pull/12) | `feat/op-run-op-check-ref` | `op_run` + `op_check_ref` + `secret-ref.ts` | Additive, non-breaking; allow-list default genericized to unrestricted |
| [#13](https://github.com/CakeRepository/1Password-MCP/pull/13) | `feat/reveal-default-hardening` | `password_read` default `reveal` → `false` | Breaking; complements #12, reviewable independently |

Both branches exist on `origin` (the fork) and target `upstream:master`.
...
- **Trigger:** pushing a git tag matching `v*` (e.g. `v2.4.3`) from
  `main`. No manual GitHub Release is required.
